...
|---|---|---|---|
| **Utenti** (account, ruoli, permessi, accessi) | **IAM** → Utenti. Dal 17/09/2026 è un albero «Lista utenti» con una riga per **persona** del registro (nome, RUI, stato ATTIVO/INATTIVO/SOSPESO) e un ingranaggio che apre **tre gruppi**: 1 che cosa vede in IAM (ruolo, profilo, prodotti, spunte), 2 compagnie visibili su Quoto (`compagnie`, `accesso_quoto`), 3 attivazione IAM (`accesso_iam`, collegamento «imposta la password» via `resetPasswordForEmail`, mai una password nel messaggio). Una persona inserita in Collaboratori compare da sola, INATTIVA finché non ha un account; **«Attiva l'accesso»** la fa passare dal server: `POST api.withusassicurazioni.it/utenti/attiva` `{persona_id, ruolo}` (solo admin, `server/utenti.js`) crea l'utenza con la chiave di servizio e una password che nessuno vede, la riga `iam_utenti` (`accesso_iam` sì, `accesso_quoto` no), scrive `iam_id` sulla persona e manda via Brevo il collegamento *recovery* verso `https://iam.withusassicurazioni.it/` per scegliere la password. Il vecchio «Nuovo utente» con la password temporanea non c'è più | `iam_utenti`, `quote_collaboratori.iam_id` | QUOTO **legge** l'elenco (`caricaUtentiIam`, per «assegna a» e per i punti vendita) e rimanda a IAM con `quoto-apri` |
...
controlla la lista dalle persone e i tre gruppi, `server/verifica/utenti-attiva.test.mjs` l'attivazione dal server (admin soltanto, password mai in uscita, 409 quando un account c'è già), `server/verifica/compagnie-utente.test.mjs` il filtro delle compagnie in QUOTO (compagnie salvate solo se il
