...
//
//  Ordina le aree da approfondire. NON decide l'adeguatezza, non propone un
//  prodotto e non sostituisce il consulente: dice da dove cominciare a
//  guardare, e dice perché.
//
//  Regole di questo file, che valgono più della comodità:
//
//  1. Stesso questionario, stesso risultato. Nessun caso, nessuna data «adesso»
//     nascosta dentro: la data di riferimento si passa da fuori. Un rating che
//     cambia da solo non si può firmare.
//  2. Ogni punteggio porta i suoi motivi. Un numero senza il perché non è
//     consulenza.
//  3. Chi cambia le regole cambia VERSIONE_REGOLE. Il risultato archiviato
//     dice con quali regole è nato, altrimenti un report di sei mesi fa non è
//     più rileggibile.
//
//  Il calcolo autorevole è quello del server: questa copia serve all'anteprima
//  mentre si compila. Le due devono restare identiche — se divergono, vale il
//  server, ed è il server a scrivere lo snapshot.
//
//  Adattato da docs/analisi-bisogni/reference/analisi-bisogni-rating.mjs, che
//  è la specifica. Le differenze sono solo di forma (qui si espone anche su
//  window per l'anteprima nel browser, che non ha un passo di costruzione).
// ═══════════════════════════════════════════════════════════════════════════════
...
/* IAM non ha un passo di costruzione: index.html carica questo file come
   modulo e le funzioni della schermata lo trovano qui. */
if (typeof window !== 'undefined') {
  window.ABRating = { VERSIONE_REGOLE, VERSIONE_QUESTIONARIO, META_CATEGORIE, calcolaNecessita, calcolaIndiceComplessivo, creaSnapshotRating };
}
